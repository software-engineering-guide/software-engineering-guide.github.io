...
The delivery pipeline is the flow of work that turns a validated idea into running software in users' hands (reliably, repeatably, and measurably) and then feeds the resulting outcome data back into discovery (chapter 11.1). It is the industrialised path from a code commit to a production change to a measured effect on users and the business. Where discovery answers *what and why*, delivery answers *how we ship it safely, how fast, and whether it actually worked*.
...
The delivery pipeline's final act is not deployment; it is **evidence**. Outcome metrics (did activation rise, did checkout time fall, did support tickets drop) flow back into the discovery pipeline (chapter 11.1) as the basis for the next round of bets. When discovery and delivery are joined by this feedback loop, the organisation becomes a learning system: hypotheses are shipped, measured, and either scaled or reversed, continuously.
...
- **Level 5, Orchestrate:** Delivery is continuously improved and integrated with discovery and risk planning. Continuous deployment runs where appropriate with progressive delivery and automated rollback; features ship as measured experiments whose outcome metrics loop back to the next round of bets; elite DORA performance is sustained across teams via the paved road; and the organisation adaptively re-tunes gates, thresholds, and capacity as load, risk, and the product mix shift.
