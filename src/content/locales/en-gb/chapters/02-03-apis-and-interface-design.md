...
- Optimise for the consumer's experience, not your own convenience.
...
- **Inconsistent conventions:** every endpoint invents its own naming, error format, and pagination, so clients cannot generalise.
...
- **Level 5, Orchestrate:** API strategy is continuously improved and integrated across the organisation; the catalogue, gateway, versioning rules, and compatibility gates work as one system; the organisation routinely retires, consolidates, and re-scopes interfaces based on measured adoption and cost; interaction-style and versioning standards adapt as the ecosystem, partners, and technology shift, and breaking changes are rare and well-managed.
