...
- **Big-bang delivery:** everything integrated and released at the end, maximising risk (contrast chapter 11.2).
...
- **Level 2 (Develop):** Basic planning, status reporting, and a risk list exist on some projects but not others; the delivery method is chosen by habit rather than fit; estimation and dependency tracking vary team by team, so practice is inconsistent across the organisation.
- **Level 3 (Standardise):** A documented approach is enforced org-wide: the delivery method is chosen to fit the work, scope is managed against the triple constraint, a living risk register and dependency map are expected on every project, and estimates are range-based and re-forecast with engaged stakeholders.
