...
- **Own your instrumentation.** Standardise on open, vendor-neutral telemetry so you control your data and can switch backends.
...
### Standardise on OpenTelemetry and structured logging
...
**Enterprise.** The work is governance across many teams: a shared OpenTelemetry standard, a common correlation-ID scheme, and curated SLO dashboards so a single responder can follow a request across dozens of services. Manage telemetry as a cost centre with retention tiers and sampling policy, standardise alerting on SLO burn rates to keep a shared on-call sustainable, and prune noisy alerts centrally so one team's fatigue does not desensitise everyone. Treat the instrumentation layer as vendor-neutral infrastructure that outlives any single backend contract.

**Government.** Procurement rules, transparency, and public accountability shape the design. Standardise on open instrumentation so a system expected to run for decades survives re-procurement by different vendors without being held hostage to proprietary agents, and require that portability in the contract. Use structured audit logs to show who accessed which record and when, redact or tokenise personal data before it reaches the telemetry store, and reconcile retention with data-minimisation law. Publish SLO dashboards for citizen-facing services so the same signals your engineers watch are visible evidence of the commitments you are held to.
...
**Government.** A national health service runs a patient-records platform under strict audit and privacy rules. Structured logs capture who accessed which record and when, feeding both security monitoring and compliance reporting, while personally identifiable fields are redacted or tokenised in telemetry. Public SLO dashboards show availability and latency for citizen-facing appointment booking. By standardising on open instrumentation, the agency avoids proprietary lock-in across a system expected to run for decades and to be re-procured by different vendors over its life.
...
- How do you preserve high-cardinality debugging power while meeting privacy and data-minimisation requirements?
...
- Standardise on OpenTelemetry and structured logging to stay vendor-neutral and portable across long system lifetimes.
