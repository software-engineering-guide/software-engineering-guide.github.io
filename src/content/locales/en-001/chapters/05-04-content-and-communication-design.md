...
| Decision | Pros | Cons |
|---|---|---|
| Centralised content strategy | Consistency, trust, reuse | Overhead, can slow teams if it becomes a bottleneck |
...
The core tension is between brand personality and clarity, and between engagement and respect for attention. Context resolves it. Let voice add warmth where stakes are low, and prioritise plain, calm clarity where stakes are high (errors, money, security, legal). On defaults and messaging, the ethical line is simple: does the design serve the user's genuine interest, or exploit their inattention?
...
2. **Is our content structured and reusable, or hard-coded and duplicated across screens?** Copy typed directly into a component cannot be updated consistently, cannot be reviewed, and cannot be localised without a code change, which quietly blocks every future market and every wording fix. This becomes acute at scale, where the same concept picks up three names because there is no glossary and no single source. Decide how content is stored, who owns the controlled vocabulary, and how a wording change propagates without a deploy per screen. Bring an example of a term your product spells or names inconsistently and trace how many places you would have to edit. If the answer is "search and replace by hand," your content is trapped in code.
...
**Small business.** With no content designer and a tight budget, lean on the plain-language and microcopy conventions built into the tools you already use, and buy templates for transactional email rather than crafting them from scratch. Prioritise the few screens that lose customers, an empty state, a checkout error, a cancellation flow, and keep declining as easy as accepting so you stay clear of dark-pattern law. Run a free readability checker over anything customer-facing before it ships.

**Enterprise.** The problem is coherence across many teams: a shared voice-and-tone guide, a controlled vocabulary so one concept keeps one name, and error, empty-state, and default patterns that live in the design system beside the components. Store content in version control, structured and localisable rather than hard-coded, give it owners and review, and coordinate multi-channel messaging so users are not spammed or contradicted. Govern honesty centrally so no team ships confirmshaming or pre-checked consent under growth pressure.
...
- **Hard-coded, duplicated copy**: impossible to update or localise consistently.
...
**Level 3: Standardise.** A content strategy, voice-and-tone guide, and controlled vocabulary are documented and used across teams. Plain language is the standard and, where required, compliant; errors, empty states, and defaults follow shared patterns; content is structured, reviewed, and reusable rather than hard-coded per screen.
...
**Level 5: Orchestrate.** Content is continuously improved and integrated across the organisation. Patterns live in the design system, localised and accessible by default; the controlled vocabulary keeps one name per concept everywhere; multi-channel communications are coordinated and user-controlled; and the practice adapts as regulation, audience, and channels shift, retiring and rewriting content on evidence rather than opinion.
...
- How should notification frequency and channel control be governed across the organisation?
