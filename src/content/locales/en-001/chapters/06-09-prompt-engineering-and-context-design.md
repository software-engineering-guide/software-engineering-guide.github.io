...
- **Level 3, Standardise:** Practices are documented and enforced across the organisation. Prompts are versioned, parameterised templates under mandatory code review, backed by a shared retrieval layer and a documented eval set that runs in CI and gates changes. Instructions are separated from untrusted data, tool access is least-privilege, and outputs are schema-validated and logged with their prompt version, the same way in every team.
