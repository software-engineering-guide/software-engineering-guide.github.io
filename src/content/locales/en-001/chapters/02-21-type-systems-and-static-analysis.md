...
**Small business.** With no static-analysis specialist on staff, favour a language and toolchain where good defaults come built in rather than a suite you must tune and babysit. Buy the analysis embedded in your IDE and your hosted CI instead of standing up your own platform, and keep the rule set close to the community standard so a contractor or a new hire recognises it. Treat warnings-as-errors and a small typed core as the highest-leverage moves your limited budget can make.

**Enterprise.** The work is governance across many teams: one version-controlled configuration every repository inherits, identical rules in the editor and the pipeline, and a ratcheted baseline so no team's coverage can silently fall. Standardise the analysers, track type coverage and suppression counts as portfolio metrics, and audit suppressions on a fixed cadence so machine-checked guarantees stay uniform enough for an auditor to rely on. Budget the platform team that owns the shared config, because consistency across thousands of engineers does not maintain itself.
...
- **Level 2, Develop:** A linter and, where relevant, a type checker run on some projects, but rules vary between teams, warnings do not fail the build, and escape hatches and broad suppressions are common. Some benefit is realised, yet coverage is inconsistent and trust in the tools is patchy.
- **Level 3, Standardise:** A shared, version-controlled configuration enforces type checking and linting in editor and CI with identical rules across the organisation. Warnings are errors with a ratcheted baseline, nullability and sum types are used to make illegal states unrepresentable at boundaries, and every suppression requires a documented, reviewable reason.
...
- **Level 5, Orchestrate:** Analysis is continuously improved and integrated across the organisation. Gradual typing has reached the critical modules, dataflow and security analysers run routinely, rules adapt as languages and threats evolve, and formal verification is applied deliberately to the few components whose failure would be catastrophic. The tooling, metrics, and rule set feed back into design, hiring, and procurement, so the whole organisation grows steadily safer to change.
...
- Match rigour to stakes: strong types plus layered analysers for most systems, and formal verification reserved for the rare component whose failure is catastrophic.
