...
from there can move an organisation of thousands over a few years.

This roadmap gives you principles of adoption, a maturity-based sequence for the
first 90 days through two-plus years, a prioritisation framework with a worked
...
- **Meet teams where they are.** A single maturity bar applied uniformly is
  unfair and demoralising. Sequence by each team's readiness and pain.

## Maturity-based sequencing

The horizons below are cumulative: each builds on the last. Dates are guidance,
not deadlines; a large or heavily regulated organisation may run each phase
longer. The pattern (*stabilise, then standardise, then scale, then sustain*)
holds regardless of pace.

### First 90 days: Stabilise and prove
...
### By 6 months: Standardise the winning pattern
...
- [ ] Roll the pattern to three to five more teams, prioritising by impact and
      readiness.
...
### By 12 months: Scale across the organisation
...
- [ ] Set organisation-wide baselines: SLOs for tier-1 services, security
...
- [ ] Begin deliberate legacy modernisation on the highest-risk systems using
...
Goal: the practices are "how we work," not a programme, and the organisation
...
- [ ] Manage technical debt and modernisation as a standing portfolio, not a
      one-time push.
...
Prioritise with a simple, defensible model rather than the loudest voice in the
room.
...
| Standardise ADRs across teams | 3 | 1 | 1.0 | 3.00 | Now (quick win) |
...
Large established organisations carry scale, many teams, deep legacy, and heavy
change-management overhead. Adapt the roadmap accordingly.

- **Federate, don't centralise everything.** A single central team cannot serve
...
- **Respect Conway's Law.** Your architecture will mirror your org chart. If you
  want decoupled services, you need decoupled, empowered teams; reorganise
  deliberately rather than fighting the grain.
- **Treat legacy as a portfolio.** You cannot modernise everything. Rank legacy
...
- **Beware the mandate reflex.** Large organisations default to policy memos.
...
Public-sector organisations add procurement cycles, compliance gates, contractor
management, multi-year funding, and transparency obligations. These are design
inputs, not excuses.

- **Design for the ATO from day one.** Authorisation to operate and continuous
...
- **Buy incrementally.** Multi-year, big-bang procurements institutionalise the
...
| **Losing sponsorship** | The executive champion leaves or disengages; the programme stalls. | Institutionalise the change in normal governance; build a coalition, not a single point of failure. |
...
| **Boiling the ocean on legacy** | Trying to modernise everything, delivering nothing. | Rank by risk and value; strangle the critical few, freeze the rest. |
...
**chapter 12.2** for the launch, review, and audit checklists that operationalise
each step.
