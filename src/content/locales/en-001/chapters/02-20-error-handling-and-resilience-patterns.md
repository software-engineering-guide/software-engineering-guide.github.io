...
For a public or cross-team API, this contract is part of the interface, as real as the parameter types. Design a small, stable error taxonomy: a bounded set of categories such as validation error, not found, conflict, unauthorised, dependency-unavailable, and internal error. Callers can then branch on category without parsing strings. A clear taxonomy makes error handling composable across many services, and it makes failures auditable, because every failure maps to a known, named kind.
...
- **Retry storms:** no backoff, no jitter, and no cap, so clients synchronise and hammer a recovering dependency back down.
...
- **Stringly-typed errors:** callers parsing error message text because there is no stable, categorised taxonomy to branch on.
...
- **Level 3, Standardise:** A shared error taxonomy and handling convention are documented and enforced across the organisation. Boundary validation, idempotent retries with backoff and jitter, circuit breakers, bulkheads, and error wrapping are standard, provided by common libraries, and every error feeds a unified observability pipeline.
...
- **Level 5, Orchestrate:** Resilience is integrated with delivery and risk planning and continuously improved. The taxonomy, shared libraries, and standards evolve from every incident, chaos and failure-injection experiments are routine, and the organisation adapts timeouts, breaker thresholds, degradation strategies, and boundary decisions as traffic, dependencies, and the risk picture shift.
...
6. If an auditor asked to see every failure your system experienced last month, could you produce it, categorised and with context?
