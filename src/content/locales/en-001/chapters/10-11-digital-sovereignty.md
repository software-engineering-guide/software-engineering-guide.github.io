...
| **Data localisation mandates** | Regulatory compliance; political assurance | Costly; fragments data; can reduce resilience and utility |
...
5. **Could we actually leave our primary provider inside a timeframe that matters, and when did we last rehearse it?** A credible exit is the truest measure of sovereignty, yet most exit plans live on paper and have never been run, so portability stays a hope rather than a demonstrated fact. The tension is cost and focus: rehearsing an exit, keeping workloads containerised, and holding an escrow of data and configuration all consume engineering attention that delivery pressure would rather spend elsewhere. Bring your most critical system, an honest estimate of how long a forced migration would take, the list of proprietary services it depends on, and the date of your last actual rehearsal (if any). For a large or public organisation carrying multi-year exit obligations in contract, an unrehearsed exit is a commitment you may be legally unable to meet, so treat a rehearsal cadence as part of the running cost of the system, not an optional exercise.
...
**Startup.** Speed and runway dominate, so buy sovereignty as a thin feature rather than build a sovereign stack you cannot staff. If a customer's jurisdiction is the constraint, deploy to your existing provider's in-region option, hold your own encryption keys so the operator cannot decrypt sensitive records, and keep the workload containerised so it stays portable. That closes the deal at a seed-stage cost and avoids a re-architecture you have no runway for.
...
**Enterprise.** The task is portfolio governance across many teams: a shared tiering of data by sovereignty sensitivity, a concentration-risk view of how much critical load sits on one provider or one jurisdiction, and standardised key control, portability, and exit rehearsals so each group stops making its own uncoordinated bet. Budget the higher cost and operational burden of the sensitive tier explicitly, and hold a documented, auditable stance you can show a regulator. Manage sovereignty as a living risk with metrics and review cadence, not a one-off migration.
...
**Startup.** A small health-tech startup lands its first hospital customer in Germany, which requires patient data to stay under EU jurisdiction. Instead of over-building a sovereign stack it cannot afford, the founders deploy to their existing cloud provider's EU region, hold their own encryption keys so the provider cannot decrypt the sensitive records, and keep the workload containerised so it stays portable. This buys most of the sovereignty benefit the customer needs at a cost a seed-stage team can carry, and it closes the deal without a wholesale re-architecture.
...
- **Over-rotation:** localising and self-hosting everything at ruinous cost and reduced capability, regardless of sensitivity.
...
- **Level 3 (Standardise):** Data and workloads are tiered by sovereignty sensitivity under a documented policy applied org-wide; jurisdiction is mapped; key control, portability, and open standards are required on sensitive tiers; exit plans exist and are mandated rather than optional.
...
- **Level 5 (Orchestrate):** Sovereignty is continuously improved and integrated across the organisation: the living risk framework feeds architecture, procurement, and risk planning by default, exits are routinely rehearsed, and the organisation adaptively re-scopes tiers, rebalances providers, and revises its stance as rulings, sanctions, and regulation shift.
