...
- Minimise complexity above all; the primary enemy of large-scale construction is code no one can fully understand.
- Anticipate change; construct so that likely future modifications are localised and cheap.
...
### Reuse deliberately and standardise

Reach for well-maintained, trusted libraries and internal components before rewriting foundational logic, and use them through clear interfaces (chapter 2.3). Build reusable components only when a genuine second use case exists, because generalising too early is its own form of complexity. Apply your organisation's coding standards and style (chapter 2.1) uniformly, ideally enforced by automated formatters and [linters](https://en.wikipedia.org/wiki/Lint_(software)), so the whole codebase reads as though one careful author wrote it.
...
### Choose and standardise construction tools
...
| Practice | Pros | Cons |
|---|---|---|
| Aggressive complexity minimisation | Readable, changeable, low defect rate | Can feel slow; risks over-abstraction if misapplied |
...
- **Level 3 (Standardise):** Complexity minimisation, boundary validation, explicit error handling, and testability are documented and enforced org-wide, in the pipeline and in review, so the whole codebase reads as though one careful author wrote it.
...
- **Level 5 (Orchestrate):** Construction is continuously improved and integrated across the organisation; defensive patterns, standards, and metrics feed back into refactoring and tooling; AI-assisted tools run under the same quality gates, and the practice adapts as languages, regulations, and risks change.
...
- Construction is where design becomes maintainable code; minimising complexity is its central discipline.
...
- Standardise tools and style, reuse deliberately, and build quality in rather than inspecting it afterward.
