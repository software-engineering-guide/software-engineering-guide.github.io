...
- Follow the *[principle of least surprise](https://en.wikipedia.org/wiki/Principle_of_least_astonishment)*: the layout should match what an experienced engineer would expect, so nothing has to be memorised.
...
- Organise dependencies so they flow in one direction, from stable cores toward volatile edges.
...
### Standardise editor and configuration files
...
Agree on conventions for naming folders and files (casing, separators, singular versus plural, and required suffixes such as those marking tests) and apply them uniformly. Names should reveal intent and match the domain vocabulary used elsewhere in the organisation. The goal is simple: a path should communicate meaning, so that reading a folder or file name tells you what is inside without opening it.

### Organise layers and dependencies deliberately
...
Provide *[scaffolding](https://en.wikipedia.org/wiki/Scaffold_%28programming%29)*, the automated generation of a starter project, so that new repositories begin already correct. A *template* or *cookiecutter* (a parameterised project skeleton that generates a ready-made repository from answers to a few prompts) encodes the standard layout, the README, the config files, and the [CI](https://en.wikipedia.org/wiki/Continuous_integration) setup in one place. When engineers create new services from a shared template, consistency becomes the default instead of an aspiration, and improvements to the template flow through to future projects.
...
| Choice | Pros | Cons |
|---|---|---|
| Strict organisation-wide standard layout | Instant familiarity; portable engineers; uniform tooling | Occasional poor fit for unusual projects; needs governance |
| Per-team freedom of layout | Local optimisation; high autonomy | Fragmentation; costly context switching; inconsistent tooling |
...
6. **Do our README and checked-in configuration files make our conventions active, or are they decorative?** A README is the front door and the checked-in `.editorconfig`, ignore file, and linter configuration are what make conventions self-enforcing, yet these are the first things to go stale and the last things anyone notices until an auditor or a new joiner cannot get the project to build. The tension is between a lean README that stays current and a thorough one that drifts, and between trusting people to format code correctly and letting shared configuration enforce it automatically. Bring a sample: pull five repos and check how many READMEs actually state what the project is, how to build, test, and run it, and who owns it, and how many carry the shared configuration files rather than relying on individual habits. For a large or regulated organisation, where integrators, security reviewers, and long-term maintainers read the README before anything else, treat a missing or stale front door as a defect with an owner, and check config-file presence automatically so conformance does not depend on goodwill.
...
- **The snowflake repository:** every repo organised differently, so each one must be relearned from scratch.
...
- **Level 1 (Initiate):** Each repository is organised ad hoc by its authors, reacting to whatever the moment needs; layouts vary widely; READMEs are missing or unreliable; newcomers must be walked through every repo by hand.
...
- **Level 3 (Standardise):** A documented, versioned structure standard is enforced across the organisation; new repositories are generated from shared templates carrying a standard layout, README, configuration files, and CI; deviations go through a documented exception process rather than happening silently.
...
- Organise every repository so that any engineer can navigate any codebase by expectation, following the principle of least surprise.
