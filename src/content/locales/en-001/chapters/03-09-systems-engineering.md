...
- **Engineer the whole, not the parts.** A system succeeds or fails as a whole, so optimising one subsystem in isolation can make the whole worse.
...
- **Treating software as separate.** Software teams optimising locally while ignoring hardware constraints, timing, and human operators.
