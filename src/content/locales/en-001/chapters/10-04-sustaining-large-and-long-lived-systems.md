...
- **Knowledge that lives in one head is a risk, not an asset.** Institutionalise understanding before the person leaves.
...
### Institutionalise knowledge transfer
...
| Incremental modernisation | Continuous risk reduction; keeps running | Slow; requires sustained funding and discipline |
...
**Enterprise.** Your challenge is portfolio scale: many long-lived systems, many teams, and staff who rotate over decades. Standardise team-level ownership in a service catalogue, measure bus factor across the estate, and fund continuous incremental modernisation rather than betting on big-bang rewrites. Govern end-of-life horizons centrally so no critical stack silently ages out of support, and run every sunset as an audited project with named accountability for completing the decommission.
...
**Enterprise.** A large insurer runs a policy-administration system first written decades ago and still central to its business. Rather than attempt a risky wholesale rewrite, it modularised the system behind well-defined interfaces and now replaces one component at a time, each change small and reversible. Every critical function has at least three people who can perform it. On-call is shared. Architecture decision records capture why the system works the way it does. A curated internal course brings new engineers to competence on the [legacy](https://en.wikipedia.org/wiki/Legacy_system) stack, and departing experts overlap with successors so tacit knowledge transfers by doing.
...
The adoption cost is ongoing and unglamorous: funding maintenance that produces no new features, paying for cross-training and documentation time that reduces short-term output, and investing in incremental modernisation that never makes headlines. The cost of *not* adopting is deferred and larger: rising risk as the stack ages, ballooning key-person exposure, and eventually a forced, high-risk, high-cost replacement under emergency conditions. When you make the case to leadership, reframe maintenance from "cost centre" to "risk management for systems the organisation cannot afford to lose." Present total cost of ownership across the full multi-decade life (including the sustainment and eventual decommission) rather than only the build. And emphasise this: for citizen- and customer-facing systems, sustained reliability is not overhead. It is the trust that is the actual product.
...
- **The doomed rewrite.** Betting the organisation on a wholesale replacement that discards encoded knowledge and usually overruns or fails.
- **Orphaned systems.** Critical software with no current owner, noticed only when it breaks.
- **Documentation theatre.** Volumes of docs that are outdated, unexercised, and trusted by no one.
...
**Level 2: Develop.** Basic practices appear but vary team by team. Ownership is written down for the most obvious major systems, some runbooks and documentation exist, and a few critical functions have a second capable person. Maintenance is funded but reactive, cross-training happens when someone remembers, and there is no shared way of doing any of it across the organisation.

**Level 3: Standardise.** Stewardship practices are documented and enforced org-wide. Team-level ownership is recorded in a service catalogue and survives reorganisations. Bus-factor mitigation through rotation and cross-training is a standing rule, architecture decision records and exercised runbooks are expected, modernisation is incremental by policy, and every sunset runs as a managed project with named accountability for completing the decommission.
