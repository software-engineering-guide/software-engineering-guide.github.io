...
- Change is controlled and recorded, not prevented; the goal is authorised, traceable change.
...
Write down an SCM plan that says what's under configuration control, how items are identified, how changes are proposed and approved, and how status is recorded and audited. Assign clear ownership, such as a configuration manager or an accountable team, so SCM isn't everyone's job and therefore no one's. Scale the process to the risk: a small internal tool needs light-touch control, while a safety-critical or regulated system needs formal boards and records. Anchor the plan in a recognised standard such as IEEE 828 so auditors and partners can follow it.
...
| Choice | Pros | Cons |
|---|---|---|
| Formal change control boards | Strong authorisation and audit trail; risk weighed before change | Slower throughput; overhead if applied to routine changes |
...
The central trade-off is control versus flow. Heavy change control gives strong assurance but slows delivery. Light control flows fast but weakens traceability. The answer isn't to pick one globally; it is to tier control by risk: automate routine changes through fast gates, and reserve formal boards and immutable baselines for the items where authorisation and auditability genuinely matter. The second trade-off is upfront tooling investment versus ongoing clerical cost and audit risk. Automated accounting costs more to set up and far less to live with.
...
- **Change control theatre:** a board that rubber-stamps everything, adding delay without adding real scrutiny.
...
- **Level 2 (Develop):** Basic practices exist but vary from team to team. Some systems define configuration items and a change process and version their releases, baselines exist here and there, but records are partly manual and the rigour of control is inconsistent across the organisation.
- **Level 3 (Standardise):** Practices are documented and enforced organisation-wide. A common configuration-item taxonomy, immutable baselines, tiered change control, and status accounting are established and largely automated; releases are reproducible and traceable, and audits are supported by tooling rather than by memory.
...
- Baselines are immutable reference points; change is authorised and recorded against them, not prevented.
