...
- **Pair every incentivised KPI with a guardrail.** A **guardrail metric** is a paired counter-metric that must not degrade while you push the primary one: call handle time paired with customer satisfaction, activation paired with support load. The guardrail makes cheating visibly expensive.
...
### Visualise honestly on dashboards
...
The central tension is **focus versus coverage**, sharpened by **motivation versus distortion**. You want enough measures to see the whole picture, few enough that a team can actually act on them, and each one guarded so that the act of incentivising it does not corrupt it. Resolve the tension by holding a small set of owned, outcome-weighted KPIs arranged in a tree under one north-star metric, and by pairing every incentivised measure with a guardrail. Coverage then comes from the tree's structure rather than from the sheer number of metrics on the screen.

## Questions to discuss with your team

1. **Do you have a single north-star metric, and does it measure delivered value or just activity?** A flat list of KPIs lets departments optimise conflicting local numbers, while a north-star aligns everyone on the value customers actually receive: completed transactions for a marketplace, activation for a product. Ask whether your top metric would keep rising if customers were getting *less* value, because if it can, it is vanity dressed as strategy. In a large organisation the north-star is what stops one team's win from being another team's loss, so it has to sit atop a KPI tree of the operational measures teams control. Bring your current top-line metric and try to trace it down to what each team owns. If the tree does not connect, the north-star is decoration rather than a steering mechanism.

2. **For each incentivised KPI, what is the cheapest way to game it, and what guardrail would expose that cheating?** Every metric you attach a reward or a reputation to invites optimisation of the number rather than the outcome, and the cheapest path is rarely the one you intended. Sit down and, for each KPI, deliberately design the exploit: how would a rational team make this number look good while doing less of what you actually want? Then name the counter-metric that would catch it (handle time paired with satisfaction, activation paired with support tickets, speed paired with error rate). This exercise matters most for the metrics you report upward or publish, because those carry the strongest incentives and therefore the strongest distortion pressure. If you cannot name a guardrail for a KPI, you are not ready to incentivise it yet.
...
**Enterprise.** The problem is dozens of teams optimising conflicting local numbers, so the work is a KPI tree under one north-star, precise definitions with a named source of truth, and a guardrail on every incentivised measure. Standardise definitions so a metric rolls up cleanly across business units, integrate operational SLIs, SLOs, and DORA metrics with the business KPIs, and govern the set as a portfolio that gets pruned, not just grown. Audit definitions, not just numbers, because at scale a quietly redefined metric misleads thousands of people at once.
...
The return on a good KPI comes from **focus and alignment**, which avoid the largest hidden cost in large organisations: many teams working hard, on time, on things that do not advance strategy. When the small set of measures that matter is explicit and visible, duplicated effort surfaces, contradictory local goals get reconciled before they collide, and low-value work loses its cover. A large organisation's most expensive resource is the aligned attention of its people, and the dominant return on KPIs is redirected capacity.

The cost of the *wrong* KPI is not the dashboard. It is quarters of effort optimising a number while the real outcome erodes, plus the cost of unwinding the gamed behaviour afterwards. A call centre that spent a year minimising handle time while maximising repeat contacts produced a negative return entirely through a single well-intentioned metric, and then had to rebuild both the process and the staff's trust.
...
- **Metric fixation:** behaviour optimises the number, not the outcome, because no guardrail exposes the gap.
...
- **Level 2, Develop:** KPIs exist for some teams with baselines and targets, but they are mostly outputs, definitions vary between teams, and guardrails are absent. Practice is inconsistent across the organisation, and gaming appears and goes unrecognised as gaming.
- **Level 3, Standardise:** A coherent, owned KPI set has precise definitions and a named source of truth, documented and enforced org-wide. Metrics are classified as leading or lagging and input, output, or outcome; incentivised measures are paired with guardrails; and charts follow honest-visualisation standards on every team.
...
- **Level 5, Orchestrate:** A KPI tree ties frontline measures to a single north-star metric; outcome metrics dominate the set; and operational SLIs, SLOs, and DORA and flow metrics integrate with business KPIs. The metrics a team chooses to move feed cleanly into its OKRs (chapter 11.4), and the organisation continuously retires, replaces, and re-scopes KPIs as strategy and the risk picture shift.

## Ideas for discussion

1. Which of your KPIs would keep rising even if the product or service were getting worse?
2. For each incentivised KPI, what is the cheapest way to game it, and what guardrail would expose that cheating?
...
- Assume **Goodhart's law**: pair every incentivised KPI with a **guardrail**, and prefer **ratios and cohorts** over raw counts.
- Reject **vanity metrics**; organise KPIs into a **tree** under a single **north-star metric**; give each a **baseline, target, and thresholds**.
- Visualise **honestly** (chapters 7.3 and 7.4), and reuse operational vocabularies: **SLIs, SLOs, and error budgets** (chapter 9.1) and **DORA and flow metrics** (chapter 11.2).
- In government, treat **published measures** with extra definitional rigour and audit the definition, not just the number.
