...
- **Prioritisation frameworks are aids to judgment, not oracles.** Numbers inform the call; they do not make it.
...
As the number of product teams grows, invest in **product operations**: the shared research, data, tooling, and practices that let many teams do discovery well without each reinventing it. Product ops keeps the customer-interview cadence staffed, the analytics trustworthy, the roadmap format consistent, and the OKR rhythm running. In an enterprise moving to a product operating model, and in a platform-as-product organisation where internal platforms have real internal customers, product ops is what keeps the model coherent across dozens of teams rather than letting it fragment into local habits.
...
| **Prioritisation by framework score** | Structured, discussable, reduces politics | False precision; can launder a bad bet as objective |
...
**Enterprise.** The work is coordinating many product teams without letting the model fragment: durable trios owning outcomes, a consistent roadmap format, and product operations keeping the interview cadence, analytics, and OKR rhythm coherent across dozens of teams. Governance and audit want traceability, so make outcomes, prioritisation rationale, and kill decisions legible rather than reverting to dated feature promises that satisfy a committee but reward output over impact. Manage the shift from project funding to a product operating model deliberately, because org design and budgeting change slower than job titles.
...
- **HiPPO-driven prioritisation:** the highest-paid person's opinion overrides evidence, and frameworks become theatre to ratify it.
...
- **Level 2, Develop:** Basic product practices appear but are inconsistent across teams. Outcomes and OKRs exist for some teams, yet goals are often output-shaped and roadmaps are still feature lists. Discovery happens occasionally, usually as an up-front phase, and prioritisation uses a framework, sometimes as cover for the loudest voice.
- **Level 3, Standardise:** Empowered trios own outcomes, and the practice is documented and expected org-wide. Roadmaps are now/next/later statements of intent; continuous discovery is a staffed, weekly habit with documented assumption tests; prioritisation frameworks inform judgment rather than replace it; and product-market fit is understood and tracked as a shared standard rather than a local habit.
...
- **Level 5, Orchestrate:** A product operating model runs across the portfolio and is continuously improved and integrated with funding and strategy. Durable teams own outcomes over years and internal platforms are managed as products; discovery and delivery loop continuously; outcome data steers investment and rebalances the portfolio adaptively; product operations keeps the practice coherent at scale; and leadership manages a portfolio of outcomes, routinely retiring, re-scoping, and reprioritising bets as evidence and the market shift.
...
- Use **prioritisation frameworks** (RICE, weighted scoring, cost of delay, Kano) as aids to judgment, never as oracles.
