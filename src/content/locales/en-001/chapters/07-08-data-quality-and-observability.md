...
The total cost of ownership includes tooling for testing, observability, and cataloguing, plus the engineering time to instrument pipelines and the organizational work of assigning owners and writing contracts. This is real, but weigh it against the cost of not doing it: silent corruption discovered by executives, machine learning models trained on bad features that encode errors at scale, analysts quietly rebuilding shadow datasets because they no longer trust the official ones, and, in regulated or public settings, correction notices that damage credibility for years. To leadership, frame data quality as insurance on every data-driven decision the organisation makes. The premium is modest and predictable. The uninsured loss, a single high-profile wrong number, is neither.
...
- **Level 2, Develop:** Some teams add basic tests on their critical tables (keys, nulls, accepted values) and a little freshness and volume monitoring on the datasets they care about most. The practices work where they exist, but coverage and rigour vary team by team, nothing is standardised, and incidents are still handled reactively.
