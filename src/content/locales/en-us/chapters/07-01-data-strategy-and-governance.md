...
1. Initiate: Data is undocumented and unowned, handled ad hoc and reactively. Definitions conflict across teams. Quality is discovered by consumers when reports break. No catalog or lineage exists.
2. Develop: Basic practices appear but are inconsistent across teams. Some datasets have owners and documentation, and a partial catalog exists. Quality checks are manual and reactive. A governance policy is written but weakly and unevenly enforced.
3. Standardize: Ownership, contracts, and SLAs are documented and enforced org-wide. Critical data products have named owners; a catalog with automated lineage covers key domains; master data exists for core entities; governance is federated with central enablement and applied consistently rather than team by team.
4. Manage: The estate is measured and controlled against baselines. Quality dimensions (accuracy, completeness, timeliness, validity, uniqueness) are tracked against published SLA targets; contract-breach rates, lineage and catalog coverage, freshness, and time-to-honor a deletion request are reported on dashboards; observability alerts on schema drift and volume anomalies; incidents get triage, root-cause analysis, and postmortems; access and go/no-go decisions rest on metrics against baselines, not opinion.
