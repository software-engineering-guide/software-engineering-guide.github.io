...
**Startup.** Make the cheap architectural choices on day one and stop there: UTF-8 end to end, every user-facing string in a message catalog, and dates, numbers, and currencies formatted through a locale-aware library. These cost almost nothing while you ship in one language and save a rewrite when your first big customer wants a second. Do not stand up a translation pipeline or support locales nobody is paying for yet; keep the door open, not the whole house furnished.
...
**Level 4: Manage.** The localization program is measured and controlled against baselines. Teams track language coverage, localization quality and defect rates, string-sync latency from commit to translated release, truncation and right-to-left rendering defects caught per release, translation cost per locale, and time to launch a new locale, and these metrics gate releases and drive where to invest human review versus machine translation.
