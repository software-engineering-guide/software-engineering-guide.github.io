...
**Small business.** With no static-analysis specialist on staff, favor a language and toolchain where good defaults come built in rather than a suite you must tune and babysit. Buy the analysis embedded in your IDE and your hosted CI instead of standing up your own platform, and keep the rule set close to the community standard so a contractor or a new hire recognizes it. Treat warnings-as-errors and a small typed core as the highest-leverage moves your limited budget can make.

**Enterprise.** The work is governance across many teams: one version-controlled configuration every repository inherits, identical rules in the editor and the pipeline, and a ratcheted baseline so no team's coverage can silently fall. Standardize the analyzers, track type coverage and suppression counts as portfolio metrics, and audit suppressions on a fixed cadence so machine-checked guarantees stay uniform enough for an auditor to rely on. Budget the platform team that owns the shared config, because consistency across thousands of engineers does not maintain itself.
