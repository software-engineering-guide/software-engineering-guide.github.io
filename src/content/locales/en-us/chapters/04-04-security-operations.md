...
**Level 1: Initiate.** Security operations are ad hoc and reactive. Security testing is manual and rare, and there is no central logging or SIEM. No incident plan exists, so response is improvised in the moment. Patching happens only when a headline forces it, and defenses are never adversarially tested.

**Level 2: Develop.** Basic practices appear but are inconsistent across teams. Some pipelines run scanners while others run none, and central logging exists in patches. A basic incident plan is documented but rarely rehearsed, patching follows loose timelines, and an annual pentest satisfies compliance without changing much. Coverage and rigor depend on which team you ask.
