...
4. **Manage.** The portfolio is measured against baselines: retrieval recall, hallucination and refusal rates, injection-defense coverage, per-call cost and latency, and escalation and correction rates are tracked on dashboards; release gates and kill criteria fire on evidence rather than opinion, and a regression run blocks any change that moves a metric the wrong way.
5. **Orchestrate.** Continuous offline and online evaluation is tied to business outcomes; injection defenses, agents, and grounding are governed and observable; the organization routinely retires, retunes, and re-scopes LLM features, and swaps models as quality, cost, and risk shift.
