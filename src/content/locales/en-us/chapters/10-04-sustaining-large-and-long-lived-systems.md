...
**Level 3: Standardize.** Stewardship practices are documented and enforced org-wide. Team-level ownership is recorded in a service catalog and survives reorganizations. Bus-factor mitigation through rotation and cross-training is a standing rule, architecture decision records and exercised runbooks are expected, modernization is incremental by policy, and every sunset runs as a managed project with named accountability for completing the decommission.
