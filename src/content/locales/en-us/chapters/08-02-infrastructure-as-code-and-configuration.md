...
**Small business.** With no dedicated platform specialist, lean on managed services and whatever IaC your cloud provider or vendor already supports rather than standing up bespoke tooling you cannot maintain. Favor buying a hosted platform whose sensible defaults (encryption, backups, patching) are handled for you over building a golden-image pipeline you have no one to run. Frame the goal narrowly: get your handful of critical resources into code so you can rebuild them after a failure or a departing contractor.
