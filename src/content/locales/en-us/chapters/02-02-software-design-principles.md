...
**Startup.** Favor the simplest design that ships and keep one well-factored module until a real second use case forces a seam. Your scarcest resource is engineering attention, so premature interfaces, layers, and speculative frameworks are pure cost. Follow the rule of three before extracting any shared abstraction, and let YAGNI kill the extension points nobody has asked for yet.
...
- **Anaemic or god objects:** models with no behavior, or objects that do everything; both signal misplaced responsibilities.
