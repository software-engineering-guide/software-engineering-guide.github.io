...
**Small business.** You have no dependency specialist and a tight budget, so buy the discipline rather than build it. Rely on the update automation your hosting and code platform already provide, favor a small set of mature libraries so upgrades stay cheap, and use a free SBOM generator in your pipeline so you can answer "are we affected?" without staffing for it. Spend your scarce attention on license checks and on not adopting trivial packages you could write in a dozen lines yourself.

**Enterprise.** The problem is consistency across many teams: a shared internal registry that mirrors the public ecosystem and enforces license, source, and version policy at one chokepoint, plus a curated set of golden libraries as the default and a documented exception path for anything else. Emit an SBOM per build into a central store so one query answers your exposure across the whole estate, roll coordinated upgrades through automated pull requests, and treat dependency health as a measured, governed portfolio rather than a per-repo accident.

**Government.** Procurement rules and public accountability shape everything. Require vendors to deliver a machine-readable SBOM and SLSA-aligned build provenance with every release, install internally only from an approved software list served by a mirror with no direct path to the public internet, and favor dependencies with stable maintenance and clear licensing because a system may run for fifteen years and must be patchable for all of them. Plan end-of-life migrations deliberately rather than as emergencies, and keep the records that let an auditor trace any shipped artifact back to its source.
...
- **Level 2, Develop:** Some teams commit lockfiles and get mostly reproducible builds, and a little automation opens update pull requests, but practice is inconsistent from repository to repository. Awareness of licenses and transitive risk is informal, with no shared policy, inventory, or control over where packages come from.
- **Level 3, Standardize:** Practices are documented and enforced org-wide. An automated updater runs on a steady cadence with sensible batching, builds install strictly from lockfiles and fail when the lockfile and manifest disagree, an SBOM is generated per build, an internal registry enforces source and license policy, and new dependencies are evaluated against a written checklist that every team applies.
