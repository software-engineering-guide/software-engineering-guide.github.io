...
4. **How much modeling do we do before we write code, and does that dose change with the uncertainty in front of us?** Big design up front and no design at all are both failure modes, and the right dose sits between them, governed by how much uncertainty a model actually retires. On a large team the pressure runs both ways: a governance process may demand a full set of diagrams before any code, locking in decisions made with the least information, while delivery pressure may push a team to skip the one state machine that would have caught a costly edge case. Bring your last two projects and sort the models you produced into those that informed a real decision and those drawn only because a template asked for them. In enterprise and government programs, where a phase gate or approval board often mandates documents up front, come ready to argue for modeling that tracks risk rather than a fixed deliverable list, so the payments core gets its rigor and the internal reporting tool does not drown in diagrams nobody reads.
...
- **Level 1 (Initiate):** Modeling is ad hoc or absent and purely reactive; no method is named; models, when drawn at all, are inconsistent, unanalyzed, and abandoned as soon as the meeting ends.
- **Level 2 (Develop):** Some teams draw common diagrams and follow a named method, but practice is uneven across the organization: models are often produced ceremonially, drift from the code, and are rarely analyzed for defects.
