...
**Level 3: Standardize.** A platform team runs documented golden paths, self-service provisioning, a developer portal with a service catalog, and scorecards, applied org-wide. Guardrails for security, policy, and compliance are embedded in the paved roads, so the standard workflow is the compliant one, and the same conventions hold across teams rather than varying by group.
