...
The cost of the *wrong* KPI is not the dashboard. It is quarters of effort optimizing a number while the real outcome erodes, plus the cost of unwinding the gamed behavior afterwards. A call center that spent a year minimizing handle time while maximizing repeat contacts produced a negative return entirely through a single well-intentioned metric, and then had to rebuild both the process and the staff's trust.
